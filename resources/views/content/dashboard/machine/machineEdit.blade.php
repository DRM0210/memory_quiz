...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-xxl">
            <div class="card mb-4">
                <h4 class="card-header">Machine Edit<span class="float-end"><a
                            href="{{ route('client-view', $data->client_id) }}" class="btn btn-success">Go Back</a></span>
                </h4>
                ...
                <div class="card-body">
                    ...
                    @endif
                    <form action="{{ route('machine-update', ['id' => $data->client_id, 'id1' => $data->id]) }}"
                        enctype="multipart/form-data" method="post">
                        @csrf
                        <input type="hidden" name="client_id" value="{{ $data->client_id }}">
                        <input type="hidden" name="machine_type" value="{{ $data->machine_type }}">
                        <input type="hidden" name="department_id" value="{{ $data->department_id }}">

                        <div class="row">

                            <div class="col-sm-6 form-group mb-5">
                                <label for="add_type">Product Category <span class="text-danger">*</span></label>
                                <select name="add_type" id="add_type" class="form-select">
                                    <option value="" disabled>Select</option>
                                    <option value="EWB" {{ $data->add_type == 'EWB' ? 'selected' : '' }}>EWB</option>
                                    <option value="Platform Scale"
                                        {{ $data->add_type == 'Platform Scale' ? 'selected' : '' }}>
                                        Platform Scale</option>
                                    <option value="Precision Scale"
                                        {{ $data->add_type == 'Precision Scale' ? 'selected' : '' }}>Precision Scale
                                    </option>
                                    <option value="Crane Scale" {{ $data->add_type == 'Crane Scale' ? 'selected' : '' }}>
                                        Crane
                                        Scale</option>
                                </select>
                            ...
                            <div class="col-sm-6 form-group mb-3">
                                ...
                                <div class="d-flex">
                                    @php
                                        $offerDetails = json_decode($data->offer_details, true) ?? [];
                                        $offerDetailsFiles = json_decode($data->offer_details_file, true) ?? [];
                                    @endphp
                                    @if (!empty($offerDetails))
                                        @foreach ($offerDetails as $detail)
                                            <input type="text" name="offer_details[]" class="form-control ms-1"
                                                value="{{ $detail }}" readonly />
                                        @endforeach
                                    @endif
                                    <div class="file-input-container">
                                        <input type="file" multiple name="offer_details_file[]"
                                            class="form-control ms-1 file-input" id="fileInputPDF1"
                                            value="{{ $data->offer_details_file }}" />
                                        <span class="file-placeholder" id="filePlaceholderPDF1">PDF
                                        </span>
                                        @if (!empty($offerDetailsFiles))
                                            @foreach ($offerDetailsFiles as $offerDetailsFile)
                                                <a href="{{ asset($offerDetailsFile) }}" target="_blank"
                            ...
                            <div class="col-sm-6 form-group mb-3">
                                ...
                                <input type="text" name="make_model" class="form-control"
                                    value="{{ old('make_model', $data->make_model) }}" />
                            ...
                            <div class="col-sm-6 form-group mb-3">
                                ...
                                <div class="d-flex">
                                    @php
                                        $poDetails = json_decode($data->po_details, true) ?? [];
                                        $poDetailsFiles = json_decode($data->po_details_file, true) ?? [];
                                    @endphp
                                    @if (!empty($poDetails))
                                        @foreach ($poDetails as $detail)
                                            <input type="text" name="po_details[]" class="form-control ms-1"
                                                value="{{ $detail }}" readonly />
                                        @endforeach
                                    @endif
                                    <div class="file-input-container">
                                        <input type="file" multiple name="po_details_file[]"
                                            class="form-control file-input ms-1" id="fileInputPDF2"
                                            value="{{ $data->po_details_file }}" />
                                        <span class="file-placeholder" id="filePlaceholderPDF2">PDF
                                        </span>
                                        @if (!empty($poDetailsFiles))
                                            @foreach ($poDetailsFiles as $poDetailsFile)
                                                <a href="{{ asset($poDetailsFile) }}" target="_blank"
                                                    rel="noopener noreferrer">View</a>
                                            @endforeach
                                        @endif
                            ...
                            <div class="col-sm-6 form-group mb-3">
                                ...
                                <div class="d-flex">
                                    @php
                                        $invoiceDetails = json_decode($data->invoice, true) ?? [];
                                        $invoiceFiles = json_decode($data->invoice_file, true) ?? [];
                                    @endphp
                                    @if (!empty($invoiceDetails))
                                        @foreach ($invoiceDetails as $detail)
                                            <input type="text" name="invoice[]" class="form-control ms-1"
                                                value="{{ $detail }}" readonly />
                                        @endforeach
                                    @endif
                                    ...
                                    <div class="file-input-container">
                                        <input type="file" multiple name="invoice_file[]"
                                            class="form-control file-input ms-1" id="fileInputPDF3"
                                            value="{{ $data->invoice_file }}" />
                                        <span class="file-placeholder" id="filePlaceholderPDF3">PDF
                                        </span>
                                        @if (!empty($invoiceFiles))
                                            @foreach ($invoiceFiles as $invoiceFile)
                                                <a href="{{ asset($invoiceFile) }}" target="_blank"
                                                    rel="noopener noreferrer">View</a>
                                            @endforeach
                                        @endif
                        ...
                        <div class="row mt-5">
                            ...
                            <div class="col-sm-12 form-group mb-3 megaField">
                                ...
                                <input type="text" name="platform_size" class="form-control"
                                    value="{{ old('platform_size', $data->platform_size) }}" placeholder="Size" />
                                <input type="text" name="platform_max_capacity" class="form-control ms-1"
                                    value="{{ old('platform_max_capacity', $data->platform_max_capacity) }}"
                                    placeholder="Max Capacity" />
                                <input type="text" name="platform_min_capacity" class="form-control ms-1"
                                    value="{{ old('platform_min_capacity', $data->platform_min_capacity) }}"
                                    placeholder="Min Capacity" />
                                <input type="text" name="platform_least_count" class="form-control ms-1"
                                    value="{{ old('platform_least_count', $data->platform_least_count) }}"
                                    placeholder="Least Count" />
                                @if ($errors->has('platform_size'))
                                    <span class="invalid-feedback" style="display: block;" role="alert">
                                        <strong>{{ $errors->first('platform_size', $data->name) }}</strong>
                                    </span>
                                @endif
                            </div>
                            ...
                            <div class="col-sm-12 form-group mb-3 megaField">
                                ...
                                <input type="text" name="loadcell_modal" class="form-control"
                                    value="{{ old('loadcell_modal', $data->loadcell_modal) }}"
                                    placeholder="Make/ Model" />
                                <input type="text" name="loadcell_type" class="form-control ms-1"
                                    value="{{ old('loadcell_type', $data->loadcell_type) }}" placeholder="Type" />
                                <input type="text" name="loadcell_capacity" class="form-control ms-1"
                                    value="{{ old('loadcell_capacity', $data->loadcell_capacity) }}"
                                    placeholder="Capacity" />
                                <input type="text" name="loadcell_serial_no" class="form-control ms-1"
                                    value="{{ old('loadcell_serial_no', $data->loadcell_serial_no) }}"
                            ...
                            <div class="col-sm-12 form-group mb-3 megaField">
                                ...
                                <input type="text" name="system_modal" class="form-control"
                                    value="{{ old('system_modal', $data->system_modal) }}" placeholder="Make/ Model" />
                                <input type="text" name="system_type" class="form-control ms-1"
                                    value="{{ old('system_type', $data->system_type) }}" placeholder="Type" />
                                <input type="text" name="system_cables" class="form-control ms-1"
                                    value="{{ old('system_cables', $data->system_cables) }}" placeholder="Cables" />
                                <input type="text" name="system_least_count" class="form-control ms-1"
                                    value="{{ old('system_least_count', $data->system_least_count) }}" placeholder="" />
                            ...
                            <div class="col-sm-12 form-group mb-3 megaField">
                                ...
                                <input type="text" name="jb_modal" class="form-control"
                                    value="{{ old('jb_modal', $data->jb_modal) }}" placeholder="Make / Model" />
                                <input type="text" name="jb_ports" class="form-control ms-1"
                                    value="{{ old('jb_ports', $data->jb_ports) }}" placeholder="Ports" />
                        ...
                        @php
                            $inclusion = json_decode($data->inclusion, true) ?? [];
                            $exclusion = json_decode($data->exclusion, true) ?? [];
                            $additionalData = json_decode($data->additional, true) ?? [];
                            $inclusionData = json_decode($data->inclusionAdditional, true) ?? [];
                            $exclusionData = json_decode($data->exclusionAdditional, true) ?? [];

                        @endphp

                        <div class="col-sm-12 form-group megaField pt-5 mb-3">
                            <label class="text-block">Include</label>
                            <div class="form-group d-flex">
                                <input type="text" name="inclusion[label]" class="form-control" placeholder="Label"
                                ...
                                <div class="date-input-container ms-1">
                                    <input type="date" name="inclusion[start_date]" class="form-control date-input"
                                        placeholder="start date" value="{{ $inclusion['start_date'] ?? '' }}" />
                                    <span class="date-placeholder">{{ $inclusion['start_date'] ?? '' }}</span>
                                </div>
                                <div class="date-input-container ms-1">
                                    <input type="date" name="inclusion[end_date]" class="form-control date-input"
                                        placeholder="end date" value="{{ $inclusion['end_date'] ?? '' }}" />
                                    <span class="date-placeholder">{{ $inclusion['end_date'] ?? '' }}</span>
                                </div>
                                <div class="file-input-container ms-1">
                                    <input type="file" multiple name="inclusion[pdf][]"
                                        ...
                                        value="{{ $inclusion['pdf'] ?? '' }}" />
                                    <span class="file-placeholder" id="filePlaceholderR1">PDF
                                    </span>

                                </div>
                                {{-- @if (!empty(json_decode($inclusion['pdf'], true)))
                                    @foreach (json_decode($inclusion['pdf'], true) as $pdf)
                                        <a class="ms-2" href="{{ $pdf }}" target="_blank"
                                            rel="noopener noreferrer">View</a><br>
                                    @endforeach
                                @endif --}}
                                    ...
                                    @for ($i = 0; $i < count($inclusionData['label']); $i++)
                                        <div class="col-sm-12 form-group mb-3 mt-2 d-flex">
                                            <input type="text" name="inclusionAdditional[label][]"
                                                class="form-control mb-2 includeData" placeholder="Label"
                                                value="{{ $inclusionData['label'][$i] ?? '' }}" />
                                            <input type="text" name="inclusionAdditional[value][]"
                                                class="form-control ms-1" value="{{ $inclusionData['value'][$i] ?? '' }}"
                                                placeholder="Value" />
                                            <button type="button" class="btn btn-danger btn-sm deleteBtn ms-1"><i
                        ...
                        <div class="col-sm-12 form-group my-3 megaField">
                            <label class="text-block">Exclude</label>
                            <div class="form-group d-flex">
                                <input type="text" name="exclusion[label]" class="form-contro" placeholder="Label"
                                    value="{{ $exclusion['label'] ?? '' }}" />
                                <div class="date-input-container ms-1">
                                    <input type="date" name="exclusion[start_date]" class="form-control date-input"
                                        placeholder="start date" value="{{ $exclusion['start_date'] ?? '' }}" />
                                    <span class="date-placeholder">{{ $exclusion['start_date'] ?? '' }}</span>
                                </div>
                                <div class="date-input-container ms-1">
                                    <input type="date" name="exclusion[end_date]" class="form-control date-input"
                                        placeholder="end date" value="{{ $exclusion['end_date'] ?? '' }}" />
                                    <span class="date-placeholder">{{ $exclusion['end_date'] ?? '' }}</span>
                                </div>
                                <div class="file-input-container ms-1">
                                    <input type="file" multiple name="exclusion[pdf][]"
                                        class="form-control file-input" id="fileInputR11" placeholder="PDF"
                                        value="{{ $exclusion['pdf'] ?? '' }}" />
                                    <span class="file-placeholder" id="filePlaceholderR11">PDF
                                    </span>
                                </div>
                                {{-- @if (!empty(json_decode($exclusion['pdf'], true)))
                            ...
                            <div id="exclusionWrapper">
                                @if (!empty($exclusionData['label']))
                                    @for ($i = 0; $i < count($exclusionData['label']); $i++)
                                        <div class="col-sm-12 form-group mb-3 mt-2 d-flex">
                                            <input type="text" name="exclusionAdditional[label][]"
                                                class="form-control excludeData"
                                                value="{{ $exclusionData['label'][$i] ?? '' }}" placeholder="Label" />
                                            <input type="text" name="exclusionAdditional[value][]"
                                                class="form-control ms-1" value="{{ $exclusionData['value'][$i] ?? '' }}"
                                                placeholder="Value" />
                                            <button type="button" class="btn btn-danger btn-sm deleteBtn ms-1"><i
                                                    class="bx bx-trash"></i></button>
                                        </div>
                        ...
                        <div class="row mt-5">
                            <div class="col-sm-12 form-group mb-3 megaField">
                                ...
                                <textarea name="specification" class="form-control">{{ old('specification', $data->specification) }}</textarea>
                        ...
                        @php
                            $stampingFiles = json_decode($data->stamping_vc, true) ?? [];
                            $brochureFiles = json_decode($data->brochure, true) ?? [];
                            $datasheetFiles = json_decode($data->datasheet, true) ?? [];
                        @endphp
                        ...
                        <div class="row">
                            <div class="form-group mb-3 megaField">
                                ...
                                <input type="file" multiple name="stamping_vc[]" id="stamping_vc"
                                    class="form-control" value="{{ $data->stamping_vc }}" />
                                @if (!empty($stampingFiles))
                                    @foreach ($stampingFiles as $stampingFile)
                                        <a class="px-2" href="{{ asset($stampingFile) }}" target="_blank"
                                            rel="noopener noreferrer">View</a>
                                    @endforeach
                                @endif
                            ...
                            <div class="form-group mb-3 megaField">
                                ...
                                @if (!empty($brochureFiles))
                                    @foreach ($brochureFiles as $brochureFile)
                                        <a class="px-2" href="{{ asset($brochureFile) }}" target="_blank"
                                            rel="noopener noreferrer">View</a>
                                    @endforeach
                                @endif
                        ...
                        <div class="row">
                            <div class="form-group mb-3 megaField">
                                ...
                                @if (!empty($datasheetFiles))
                                    @foreach ($datasheetFiles as $datasheetFile)
                                        <a class="px-2" href="{{ asset($datasheetFile) }}" target="_blank"
                                            rel="noopener noreferrer">View</a>
                                    @endforeach
                                @endif
                            </div>
                            ...
                            <div class="row">
                                <div class="col-sm-12 form-group mb-3 megaField">
                                    ...
                                    <input type="text" name="product_link" class="form-control w-50"
                                        value="{{ old('product_link', $data->product_link) }}" />
                                ...
                                @if (!empty($additionalData['label']))
                                    @for ($i = 0; $i < count($additionalData['label']); $i++)
                                        <div class="col-sm-12 form-group mb-3 mt-2 d-flex megaField">
                                            ...
                                            <input type="text" name="additional[label][]" class="form-control ms-1"
                                                value="{{ $additionalData['label'][$i] ?? '' }}" placeholder="Label" />
                                            <select name="additional[type][]" class="form-control ms-1">
                                                <option value="" disabled>Select Type</option>
                                                <option value="Burn Defect"
                                                    {{ ($additionalData['type'][$i] ?? '') == 'Burn Defect' ? 'selected' : '' }}>
                                                    Burn Defect</option>
                                                <option value="Mfg Defect"
                                                    {{ ($additionalData['type'][$i] ?? '') == 'Mfg Defect' ? 'selected' : '' }}>
                                                    Mfg Defect</option>
                                            </select>
                                            <input type="text" name="additional[field][]" class="form-control ms-1"
                                                value="{{ $additionalData['field'][$i] ?? '' }}" placeholder="Field" />
                                            <button type="button" class="btn btn-danger btn-sm deleteBtn ms-1"><i
                                                    class="bx bx-trash"></i></button>
                                        </div>
                                    @endfor
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#addMore1').click(function() {
                let templateHtml = `
            <div class="col-sm-12 form-group mb-3 mt-2 d-flex">
                <input type="text" name="inclusionAdditional[label][]" class="form-control mb-2 includeData" placeholder="Label" />
                <input type="text" name="inclusionAdditional[value][]" class="form-control" placeholder="Value" />
                <button type="button" class="btn btn-danger btn-sm deleteBtn py-0"><i class="bx bx-trash"></i></button>
            </div>`;
            ...
            $('#addMore2').click(function() {
                let templateHtml = `
            <div class="col-sm-12 form-group mb-3 mt-2 d-flex">
                <input type="text" name="exclusionAdditional[label][]" class="form-control excludeData" placeholder="Label" />
                <input type="text" name="exclusionAdditional[value][]" class="form-control" placeholder="Value" />
                <button type="button" class="btn btn-danger btn-sm deleteBtn py-0"><i class="bx bx-trash"></i></button>
            </div>`;
            ...
            <div class="col-sm-12 form-group mb-3 mt-2 d-flex megaField">
                ...
                <select name="additional[type][]" class="form-control">
                    <option value="" disabled>Select Type</option>
                    <option value="Burn Defect">Burn Defect</option>
                    <option value="Mfg Defect">Mfg Defect</option>
                </select>
                <input type="text" name="additional[field][]" class="form-control" placeholder="Field" />
                <button type="button" class="btn btn-danger btn-sm deleteBtn py-0"><i class="bx bx-trash"></i></button>
            </div>`;
        ...
        setupFileInput('fileInputR2', 'filePlaceholderR2');
        setupFileInput('fileInputVC', 'filePlaceholderVC');
        setupFileInput('fileInputR11', 'filePlaceholderR11');
        setupFileInput('fileInputR22', 'filePlaceholderR22');
        setupFileInput('fileInputVC2', 'filePlaceholderVC2');
    </script>
@endsection
