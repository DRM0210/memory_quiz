                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-9 border p-2">
                            <span class="w-100 d-block p-1 fw-bold text-dark" style="background: #e5e5e5;">Role
                                Edit <span class="float-end"><a href="{{ route('role') }}"
                                        class="btn btn-success">Back</a></span></span>

                            <form action="{{ route('role-update', $data[0]['id']) }}" method="post">
                                @csrf
                                        ...
                                        <div class="input-group input-group-merge">
                                            @foreach ($permissions as $permission)
                                                <input type="checkbox" name="permission[]"
                                                    {{ in_array($permission->id, json_decode($data[0]['permissions'])) ? 'checked' : '' }}
                                                    value="{{ $permission->id }}" />
