...
<head>
    ...
    <title>{{ $company && $company->name ? $company->name . ' - Student Sign Up' : 'Student Sign Up' }}</title>
...
<style>
    ...
    *:focus { outline-offset: 4px; }
    ...
    .main__form { margin-inline: auto; max-width: 28rem; width: 100%; }
    .auth-form {
        color: #433532;
        display: grid;
        row-gap: var(--size-3);
    ...
    .auth-form__title { margin-top: var(--size-2); margin-bottom: var(--size-2); font-weight: var(--font-weight-6); font-size: var(--font-size-5); text-align: center; }
    .auth-form__logo { display: block; margin-inline: auto; margin-bottom: var(--size-2); max-width: 8rem; height: auto; object-fit: contain; }
    ...
    .auth-form__footer { margin-top: var(--size-3); text-align: center; font-size: 0.9rem; }
...
<body class="page">
    <main class="main page__main">
        <form class="auth-form main__form" action="{{ route('student.signup.post') }}" method="POST">
            @csrf
            <img src="{{ $company && $company->logo ? asset($company->logo) : asset('assets/img/favicon/ispl_logo.png') }}" class="auth-form__logo" alt="Logo">
            <h3 class="auth-form__title">Student Sign Up</h3>
            @if (session('success'))
                <div class="invalid-feedback" style="color: green;">{{ session('success') }}</div>
            @endif
            <label class="auth-form__label" for="name">
                <input class="auth-form__input" type="text" id="name" name="name" value="{{ old('name') }}" placeholder="Name" required/>
                @if ($errors->has('name'))<span class="invalid-feedback">{{ $errors->first('name') }}</span>@endif
            </label>
            <label class="auth-form__label" for="email">
                <input class="auth-form__input" type="email" id="email" name="email" value="{{ old('email') }}" placeholder="Email" required/>
                @if ($errors->has('email'))<span class="invalid-feedback">{{ $errors->first('email') }}</span>@endif
            </label>
            <label class="auth-form__label" for="phone">
                <input class="auth-form__input" type="text" id="phone" name="phone" value="{{ old('phone') }}" placeholder="Phone" required/>
                @if ($errors->has('phone'))<span class="invalid-feedback">{{ $errors->first('phone') }}</span>@endif
            </label>
            <label class="auth-form__label" for="address">
                <input class="auth-form__input" type="text" id="address" name="address" value="{{ old('address') }}" placeholder="Address (optional)"/>
                @if ($errors->has('address'))<span class="invalid-feedback">{{ $errors->first('address') }}</span>@endif
            </label>
            <label class="auth-form__label">
                <div class="input-wrap">
                    <input class="auth-form__input" type="password" id="password" name="password" placeholder="Password" required minlength="6"/>
                    <button type="button" class="pw-toggle" aria-label="Show password" data-target="password">
            ...
            <label class="auth-form__label">
                <div class="input-wrap">
                    <input class="auth-form__input" type="password" id="password_confirmation" name="password_confirmation" placeholder="Confirm Password" required minlength="6"/>
                    <button type="button" class="pw-toggle" aria-label="Show password" data-target="password_confirmation">
                        <svg class="eye-open" width="20" height="20" fill="currentColor" viewBox="0 0 16 16"><path d="M16 8s-3-5.5-8-5.5S0 8 0 8s3 5.5 8 5.5S16 8 16 8zM1.173 8a13.133 13.133 0 0 1 1.66-2.043C4.12 4.668 5.88 3.5 8 3.5c2.12 0 3.879 1.168 5.168 2.457A13.133 13.133 0 0 1 14.828 8c-.058.087-.122.183-.195.288-.335.48-.83 1.12-1.465 1.755C11.879 11.332 10.119 12.5 8 12.5c-2.12 0-3.879-1.168-5.168-2.457A13.134 13.134 0 0 1 1.172 8z"/><path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5zM4.5 8a3.5 3.5 0 1 1 7 0 3.5 3.5 0 0 1-7 0z"/></svg>
                        <svg class="eye-closed" width="20" height="20" fill="currentColor" viewBox="0 0 16 16" style="display:none;"><path d="M13.359 11.238C15.06 9.72 16 8 16 8s-3-5.5-8-5.5a7.028 7.028 0 0 0-2.79.588l.77.771A5.944 5.944 0 0 1 8 3.5c2.12 0 3.879 1.168 5.168 2.457A13.134 13.134 0 0 1 14.828 8c-.058.087-.122.183-.195.288-.335.48-.83 1.12-1.465 1.755C11.879 11.332 10.119 12.5 8 12.5c-2.12 0-3.879-1.168-5.168-2.457A13.134 13.134 0 0 1 1.172 8z"/><path d="M11.297 9.176a3.5 3.5 0 0 0-4.474-4.474l.823.823a2.5 2.5 0 0 1 2.829 2.829l.822.822zm-2.943 1.299.822.822a3.5 3.5 0 0 1-4.474-4.474l.823.823a2.5 2.5 0 0 0 2.829 2.829z"/><path d="M3.35 5.47c-.18.16-.353.322-.518.487A13.134 13.134 0 0 0 1.172 8l.195.288c.335.48.83 1.12 1.465 1.755C4.121 11.332 5.881 12.5 8 12.5c.716 0 1.39-.133 2.02-.36l.77.772A7.029 7.029 0 0 1 8 13.5C3 13.5 0 8 0 8s.939-1.721 2.641-3.238l.708.709z"/></svg>
                    </button>
                </div>
                @if ($errors->has('password_confirmation'))<span class="invalid-feedback">{{ $errors->first('password_confirmation') }}</span>@endif
            </label>
            <button class="primary-btn" type="submit">Sign Up</button>
            <div class="auth-form__footer">
                Already have an account? <a class="auth-form__link" href="{{ route('student.login') }}">Login</a>
            </div>
        </form>
    </main>
    <script>
